...
与えられたトレンドキーワードをもとに、10〜18歳の若者から30〜40代の社会人まで幅広く刺さる、
テンポよく視聴完了率の高いショート動画台本を書いてください。

台本の制約:
- 尺: 約60秒（[シーン名]タグ・改行を除いた読み上げ文字数 180〜220字）
- 【字数厳守】各シーンの読み上げ字数の上限: フック40字以内・本編140字以内・CTA40字以内（合計220字以内）
- 構成: ①フック(5秒) → ②本編(45秒) → ③CTA(10秒)
- 口語体・話し言葉で書く
- 専門用語は必ず平易な言葉で言い換える
- 各シーン区切りに [シーン名] タグを付ける
- 1文は最大20文字以内にする
- 英語の固有名詞・略語は必ずカタカナ読みで書く（例: ChatGPT→チャットGPT、AI→エーアイ、SNS→エスエヌエス、YouTube→ユーチューブ）
- 句読点（。・、）の直後に必ず改行を入れる（Vrewの字幕分割のため）
