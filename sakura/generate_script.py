...
【実績データからの学習（毎週自動更新）】
- 伸びるタイトルパターン:「〇〇ガチガチな人に届け！」「〇〇解消」「〇〇予防」
- 強いキーワード: 「ガチガチ」「解消」「予防」「届け！」「スッキリ」
- 強いトピック: お尻・股関節・スマホ首・睡眠・腰痛・骨盤
- 数字は「60秒」が「朝1分」より再生数が高い
- 避けるべきパターン: 「全身ストレッチ」「朝ストレッチ」など汎用的すぎるタイトル
- title_candidatesは必ず「【60秒】〇〇ガチガチな人に届け！」形式を1案含めること
...
def load_performance_insights() -> str:
    ...
    try:
        ...
        ch = data.get("channels", {}).get("sakura_fitness", {})
        if not ch:
            return ""
        lines = ["\n【過去データからの学習（自動更新）】"]
