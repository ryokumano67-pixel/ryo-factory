...
GOOGLE_TTS_VOICE   = os.getenv("GOOGLE_TTS_VOICE", "ja-JP-Neural2-B")
...
- 最後はポジティブな一言で締めた後、「詳しくは概要欄をチェックしてね！」で終わる
- 読点「、」を多めに入れて自然な間を作る
- 1文は15〜20文字を目安に短く切る
- 読み方注意: 「床」は必ず「ゆか」と読む（「とこ」は絶対に使わない）
...
CTA_PHRASE = "詳しくは概要欄をチェックしてね！"


def _ensure_cta(script: str) -> str:
    """概要欄CTAが末尾になければ強制追加する"""
    if "概要欄" in script and "チェック" in script:
        return script
...
def generate_script(topic: str) -> dict:
    ...
    try:
        ...
        if m:
            ...
            raise
    data["script"] = _ensure_cta(data["script"])
    return data


def _generate_audio_google(text: str, output_path: Path) -> Path:
    import base64
    ...
    if not resp.ok:
        raise Exception(f"Google TTS error: {resp.status_code} {resp.text}")
    audio_b64 = resp.json()["audioContent"]
    output_path.write_bytes(base64.b64decode(audio_b64))
    print(f"音声生成完了(Google TTS): {output_path}")
    return output_path


def generate_audio(text: str, output_path: Path, voice_id: str = VOICE_ID) -> Path:
    if GOOGLE_TTS_API_KEY and voice_id == VOICE_ID:
...
def run_pipeline(topic: str, script_data: dict = None, audio_url: str = None, index: int = None):
    ...
    try:
        comment_text = (
            ...
            f"→ {RAKUTEN_ROOM_URL}\n\n"
            f"🧘 ヨガ・フィットネス無料体験\n"
            f"▶ ホットヨガLAVA → https://px.a8.net/svt/ejp?a8mat=4B3K2R+201GZ6+1O96+5ZEMQ\n"
            f"▶ オンラインヨガSOELU → https://px.a8.net/svt/ejp?a8mat=4B3K2R+1THPBM+4EPM+631SY\n"
            f"▶ ライザップ → https://px.a8.net/svt/ejp?a8mat=4B3K2R+1MCI2A+3D3Q+62ENM\n\n"
            f"チャンネル登録＋🔔通知ONで毎朝6時更新！"
        )
